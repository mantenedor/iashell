...
from typing import Optional  # IMPORTANTE: Adicione esta linha
...
# CORRIGIDO: Usando Optional em vez de | para Python < 3.10

def build_runtime_context(user_text: str, doc_context: Optional[str] = None, force_mode: Optional[str] = None) -> str:
...
print("IA pronta (!comando para shell, :add <arquivo> para indexar, :docs para listar documentos, :diagnose <doc_id> para diagnóstico, q ou quit para sair)\n")
...
while True:
    ...
    try:
        ...
        if q.startswith(DIAGNOSE_PREFIX):

            doc_id = q[len(DIAGNOSE_PREFIX):].strip()

            if not doc_id:

                print("Uso: :diagnose <doc_id>\n")
        ...
        if q.startswith(INGEST_PREFIX):
            ...
            try:
                ...
                print(f"Chunks gerados: {meta.get('chunk_count', 0)}")

                print(f"Resumo: {meta.get('summary', '')[:200]}...\n")
